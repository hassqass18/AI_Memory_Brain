# Sierra Homes — American Dream Apartments
# Email Marketing Nurture Sequence (6 Emails)

> **Template:** `SierraHomes_Email_Master_Template.html`
...
## Sequence Overview & Cadence

| # | Email | Send Day | Goal |
|---|-------|----------|------|
| 1 | Welcome & Introduction | Day 0 (immediate) | Build trust, set expectations |
| 2 | Why Now — Off-Plan Advantage | Day 3 | Educate on pricing window |
| 3 | The Lifestyle — Amenities & Design | Day 6 | Create emotional desire |
| 4 | Your Floor Plan — Units & Pricing | Day 10 | Move to consideration |
| 5 | The Investment Case | Day 14 | Justify the decision financially |
| 6 | Last Call — Pre-Launch Closing | Day 18 | Drive to reservation |
...
**Send:** Day 0 (immediately on opt-in)
...
| `{{STAT_3_VALUE}}` | 22% |
| `{{STAT_3_LABEL}}` | Land Equity Already Secured |
...
## EMAIL 2 — Why Now: The Off-Plan Advantage
**Send:** Day 3
...
| Field | Value |
|-------|-------|
| `{{EMAIL_SUBJECT}}` | Why the Smartest Buyers Always Enter Off-Plan |
| `{{PREHEADER_TEXT}}` | The price you see today is the lowest it will ever be. Here's why early buyers win. |
| `{{HERO_IMAGE_URL}}` | assets/renders/10_Render_Exterior_Angle3.jpg |
| `{{BANNER_PRICE}}` | KES 10,000,000 |
| `{{BANNER_UNIT_TYPE}}` | Off-Plan vs KES 15M+ at Completion |
| `{{EMAIL_EYEBROW}}` | Pre-Launch Pricing Window |
| `{{EMAIL_HEADLINE}}` | The Best Price You'll Ever Pay for This Unit Is Right Now |
| `{{EMAIL_BODY_PARA_1}}` | Every milestone we hit — permit issued, foundation poured, structure topped out — the price goes up. That's not a sales tactic. That's how off-plan property works, everywhere in the world. The buyer who enters before the permit takes on slightly more risk, and in return, gets the lowest price the developer will ever offer. |
| `{{EMAIL_BODY_PARA_2}}` | At American Dream Apartments, a 1-bedroom is KES 10,000,000 today. By the time construction completes in August 2029, the same unit in the same building will be priced at market rates reflecting the completed, liveable, proven asset. Your upside is built in from day one. |
| `{{STAT_1_VALUE}}` | 36 |
| `{{STAT_1_LABEL}}` | Month Flexible Payment Plan |
| `{{STAT_2_VALUE}}` | 8–12% |
| `{{STAT_2_LABEL}}` | Annual Land Appreciation in Nyali |
| `{{STAT_3_VALUE}}` | KES 250K |
| `{{STAT_3_LABEL}}` | Reservation to Secure Your Unit |
...
## EMAIL 3 — The Lifestyle: Amenities & Design
**Send:** Day 6
...
## EMAIL 4 — Your Floor Plan: Units & Pricing
**Send:** Day 10
...
| Field | Value |
|-------|-------|
| `{{EMAIL_SUBJECT}}` | Why Serious Investors Are Already Moving on American Dream |
| `{{PREHEADER_TEXT}}` | Nyali has delivered 8–12% annual appreciation for a decade. Off-plan is where the returns begin. |
| `{{HERO_IMAGE_URL}}` | assets/renders/13_Render_Perspective1.jpg |
| `{{BANNER_PRICE}}` | 43.4% ROI |
| `{{BANNER_UNIT_TYPE}}` | Developer Projected Return on Cost |
| `{{EMAIL_EYEBROW}}` | The Investment Case |
| `{{EMAIL_HEADLINE}}` | This Is Where the Numbers Make Sense |
| `{{EMAIL_BODY_PARA_1}}` | Nyali has delivered 8–12% annual land value appreciation for the last decade. It is consistently the most sought-after residential corridor in Mombasa County — driven by proximity to the ocean, superior infrastructure, and sustained demand from both the diaspora and the domestic premium buyer. American Dream Apartments sits at the centre of this corridor, on a titled plot, with architectural drawings complete. |
| `{{EMAIL_BODY_PARA_2}}` | Entering at the off-plan stage — before the permit is issued — is the most strategically advantageous position in any property cycle. You are buying into a development at a price that reflects current cost inputs, not future completed-asset value. By the time your unit is handed over in August 2029, the gap between what you paid and what it's worth will be significant. |
| `{{STAT_1_VALUE}}` | 8–12% |
| `{{STAT_1_LABEL}}` | Annual Nyali Land Appreciation |
| `{{STAT_2_VALUE}}` | Aug 2029 |
| `{{STAT_2_LABEL}}` | Handover — 36-Month Build |
| `{{STAT_3_VALUE}}` | 22% |
| `{{STAT_3_LABEL}}` | Land Already Equity-Secured |
| `{{FEATURE_EYEBROW}}` | For the Diaspora Buyer |
| `{{FEATURE_ACCENT_WORD}}` | OWN MOMBASA |
| `{{FEATURE_HEADLINE_REST}}` | FROM WHEREVER YOU ARE |
| `{{FEATURE_BODY}}` | You don't need to be in Kenya to reserve your unit. Our process is fully remote — from reservation to SPA execution, everything can be handled digitally. American Dream Apartments was built with the diaspora buyer in mind: a premium asset, in a market you know, that builds value while you're building life elsewhere. When you're ready to return — or ready to rent — it's there. |
| `{{FEATURE_IMAGE_URL}}` | assets/renders/11_Render_Exterior_Angle4.jpg |
...
## EMAIL 6 — Last Call: Pre-Launch Closing
**Send:** Day 18
...
| Field | Value |
|-------|-------|
| `{{EMAIL_SUBJECT}}` | Pre-Launch Window Closing — Reserve Before Prices Rise |
| `{{PREHEADER_TEXT}}` | Once the permit lands, the price goes up. This is your last chance at pre-launch terms. |
| `{{HERO_IMAGE_URL}}` | assets/renders/12_Render_Exterior_Angle5.jpg |
| `{{BANNER_PRICE}}` | Pre-Launch Closes Soon |
| `{{BANNER_UNIT_TYPE}}` | Limited Units Remaining |
...
| `{{STAT_2_LABEL}}` | Permit Expected — Prices Revise |
| `{{STAT_3_VALUE}}` | 85 |
| `{{STAT_3_LABEL}}` | Units Available to Developer Buyers |
...
Use this table to quickly fill in `SierraHomes_Email_Master_Template.html` for any email in the sequence or for future one-off campaigns.

| Placeholder | Description |
|-------------|-------------|
| `{{EMAIL_SUBJECT}}` | Email subject line |
| `{{PREHEADER_TEXT}}` | Hidden preview text (shown in inbox before open) |
| `{{LOGO_URL}}` | Hosted URL of Sierra USA Homes logo PNG |
| `{{HERO_IMAGE_URL}}` | Full-width hero image (hosted URL or relative path) |
| `{{BANNER_PRICE}}` | Large text in right column of navy banner |
| `{{BANNER_UNIT_TYPE}}` | Small label below the banner price |
| `{{EMAIL_EYEBROW}}` | Tiny all-caps label above the headline (gold) |
| `{{EMAIL_HEADLINE}}` | Main H1 headline of the email |
| `{{EMAIL_BODY_PARA_1}}` | First body paragraph |
| `{{EMAIL_BODY_PARA_2}}` | Second body paragraph |
| `{{STAT_1_VALUE}}` | Large number/value in right stats panel |
| `{{STAT_1_LABEL}}` | Label below stat 1 |
| `{{STAT_2_VALUE}}` | Large number/value — stat 2 |
| `{{STAT_2_LABEL}}` | Label below stat 2 |
| `{{STAT_3_VALUE}}` | Large number/value — stat 3 |
| `{{STAT_3_LABEL}}` | Label below stat 3 |
| `{{FEATURE_EYEBROW}}` | Small label above the feature sub-headline |
| `{{FEATURE_ACCENT_WORD}}` | First word(s) of feature headline — rendered in gold |
| `{{FEATURE_HEADLINE_REST}}` | Remainder of feature headline — rendered in navy |
| `{{FEATURE_BODY}}` | Feature section body copy |
| `{{FEATURE_IMAGE_URL}}` | Secondary image (right column of feature block) |
| `{{FEATURE_IMAGE_ALT}}` | Alt text for secondary image |
| `{{CTA_SUBTEXT}}` | Small text above the CTA button |
| `{{CTA_LABEL}}` | Button label text |
| `{{CTA_URL}}` | Button destination URL |
| `{{UNSUBSCRIBE_URL}}` | Unsubscribe link URL |
| `{{PRIVACY_URL}}` | Privacy policy URL |
| `{{VIEW_ONLINE_URL}}` | View in browser URL |

---

## Zoho Campaigns Setup Notes

- **Template name:** `SierraHomes_Default_Email_Template`
- **Mark as default:** Yes — set in Zoho Campaigns > Templates > Set as Default

# MSIFS — Staging Master Context

**Project Name:** MSIFS (Multinational Strategic Investment Funds)
**Fund:** Frontier Impact Infrastructure & Minerals Fund (FIIMF) — Fund 1
**Category:** Investment Fund Formation
**Owner:** Andre "Hassan" Wilson — Regional CIO, East Africa
**Stage:** Staging (Active Formation — LP Conversations April 2026)
**Last Updated:** 2026-04-10

---

## Fund Overview

**Target Size:** $200M | 10-year LP | 20% carry / 8% hurdle
**Strategy:** Infrastructure, minerals, and frontier market impact investing
**Geography:** East Africa (Kenya, Tanzania, Mozambique) + Central Asia (Kazakhstan, Kyrgyzstan)

---

## GP Team

| Partner | Role | Carry |
|---------|------|-------|
| Alexander Orta | CEO/CIO — Fundraising, Strategy & Operations | 27% |
| Andre "Hassan" Wilson | Regional CIO — East Africa | 25% |
| Aibek Sabirov | Regional CIO — Central Asia | 25% |
| Stephen Habetz | CFO/COO — Financial Structuring & Compliance | 15% |

---

## Formation Status

### Documents in Development
- **Fund Prospectus:** `Multinational Strategic Investment Funds (MSIFS) Fund 1 Prospectus.docx` ✅ Draft
- **GP Agreement:** `MSIFS_GP_Agreement_DRAFT (2).docx` 🔄 In review
- **Scope of Responsibility:** `MSIFS_Scope_of_Responsibility DRAFT.docx` 🔄 In review
- **Infrastructure Finance Model:** `MSIFS_Infrastructure_Finance_Model_v2.xlsx` ✅ v2 complete
- **Advisory Board Pitch:** `MSIFS-Advisory-Board-Pitch.docx` ✅ Complete
- **Advisory Board Candidates:** `Potential Advisory Board Candidates.xlsx` ✅ Available
- **Project Overview:** `MSIFS_Project_Overview_Feb2026.docx` ✅ Background
- **GP Partnership Bios:** `GP-Branding/MSIFS-GP-Partnership-Bios.docx` ✅ Complete (April 2026) — 3-tier bios (80/175/350 words) for all 4 GPs, partnership-value frame

---

## Active Agents

| Domain | Folder | Expertise |
|--------|--------|-----------|
| Finance & Investment | `Finance/agents/` | Finance-AGENT.md, Finance-advisor.md, Investment-AGENT.md, Investment-advisor.md |
| Legal | `Legal/agents/` | Legal-AGENT.md, Legal-advisor.md |
| Marketing & BizDev | `Marketing/agents/` | Marketing-AGENT.md, Market-Development-advisor.md |
| Operations | `Operations/agents/` | Operations-AGENT.md, Operations-advisor.md |
| Executive & Strategy | `Executive/agents/` | Strategy-advisor.md, PMO-advisor.md, Growth-n-Revenue-advisor.md, Politics-advisor.md |

---

## Staging Roadmap

### Document 1: Project Context / Fund Overview
**Status:** [x] Complete — Prospectus and Project Overview drafted

### Document 2: Architecture / Fund Structure
**Status:** [~] In Progress — GP Agreement and Finance Model active

### Document 3: Release Plan / Capital Raise Plan
**Status:** [ ] Not Started — LP outreach sequencing needed

---

## Notion Integration

- **MSIFS Dashboard:** Fund overview, pipeline, GP team
- **MSIFS Action Log:** Full task board (49+ items) with assignments and priorities
- **GP Agreement Overview:** Structured summary of working draft
- **Kenya Government Engagement Strategy:** Research-backed next steps
- **Kenya / Kazakhstan:** Regional intelligence pages

---

## GP Branding (April 2026)

**Status:** Active — deep research complete, profiles populated, partnership bios written

**Folder:** `GP-Branding/` — one subfolder per GP, each is single source of truth for all branding assets

| GP | Folder | Profile Status |
|----|--------|---------------|
| Alexander Orta | `GP-Branding/Alexander-Orta/profile.md` | ✅ Complete — CERAWeek, Poets & Quants, Liquidstar, Susilo Institute, MS Quant Econ |
| Andre "Hassan" Wilson | `GP-Branding/Andre-Hassan-Wilson/profile.md` | ✅ Complete — 6 years Kazakhstan, East Africa pipeline, GeniusCo/Provecta, DISC S/I |
| Aibek Sabirov | `GP-Branding/Aibek-Sabirov/profile.md` | ✅ Complete — Kashagan/NCOC N.V., May 2026 graduation, DISC I/C, multilingual |
| Stephen Habetz | `GP-Branding/Stephen-Habetz/profile.md` | ✅ Complete — CFA Aug 2024, Moelis energy IB, Morgan Stanley, Notre Dame Meyer Fellow |

**Master Research File:** `GP-Branding/_Master-GP-Research.md` — consolidated research log, open questions, GP Branding Questionnaire draft

**Open Actions:**
- [ ] GP Branding Questionnaire: send to all 4 GPs (especially Hassan — Kazakhstan work history; Alex — military dates, Peace Corps)
- [ ] Professional headshots: none yet collected
- [ ] LinkedIn copy: headlines and about sections drafted in profiles, not yet approved/published
- [ ] Fund digital presence decision: website vs. LinkedIn-only

---

## Key Priorities (April 2026)

1. Finalize GP Agreement — get to signed status
2. Advisory Board: identify 3–5 confirmed members
3. LP outreach: first 10 qualified conversations
4. Kenya government engagement strategy execution
5. Finance model: LP waterfall and scenario modeling complete
